...
const HeroSection = () => {
  ...
  return <section className="min-h-screen flex items-center justify-center hero-gradient bg-grid relative overflow-hidden">
      ...
      <div className="relative z-10 text-center space-y-12 px-4 max-w-6xl">
        {/* Main Title */}
        <div className="space-y-8 py-[5px]">
          ...
          <div className="text-2xl md:text-4xl font-mono text-muted-foreground animate-slide-in-left min-h-[3rem] flex items-center justify-center">
            ...
            <TypingAnimation texts={typingTexts} typingSpeed={100} deletingSpeed={50} pauseDuration={2000} className="text-neon-green" />
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 animate-fade-in">
          {stats.map((stat, index) => <div key={stat.label} className="glass-card p-8 text-center hover:bg-white/10 transition-all duration-300 hover:scale-105 group border border-white/20" style={{
        ...
        <div className="flex flex-col sm:flex-row gap-8 justify-center animate-scale-in">
          ...
          <Button size="lg" variant="outline" className="ui-btn border-neon-cyan text-neon-cyan hover:bg-neon-cyan/10 transition-all duration-300 hover:scale-105 group px-8 py-4">
            <span className="flex items-center gap-3">
        ...
        }].map((social, index) => <a key={social.label} href={social.href} className={`p-6 glass-card ${social.color} transition-all duration-300 hover:scale-110 group relative overflow-hidden border border-white/20`} style={{
