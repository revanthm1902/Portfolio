...
const HeroSection = () => {
  const stats = [
    { icon: Code, value: '50+', label: 'Projects Built', color: 'neon-purple' },
    { icon: Zap, value: '3+', label: 'Years Experience', color: 'neon-cyan' },
    { icon: Award, value: '15+', label: 'Certifications', color: 'neon-green' },
    { icon: Coffee, value: '∞', label: 'Cups of Coffee', color: 'neon-purple' },
  ];
      ...
      <div className="relative z-10 text-center space-y-12 px-4 max-w-6xl">
        ...
        <div className="flex flex-wrap justify-center gap-3 animate-scale-in">
          {skills.map((skill, index) => (
            <span
              key={skill}
              className="px-4 py-2 glass-card text-sm font-mono text-neon-cyan hover:bg-neon-cyan/20 transition-all duration-300 hover:scale-110 cursor-pointer"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 animate-fade-in">
          {stats.map((stat, index) => (
            <div
              key={stat.label}
              className="glass-card p-6 text-center hover:bg-white/10 transition-all duration-300 hover:scale-105 group"
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-center animate-scale-in">
          <Button 
            size="lg" 
            className="bg-neon-purple hover:bg-neon-purple/80 text-white border border-neon-purple/50 neon-glow transition-all duration-300 hover:scale-105 group"
          >
          ...
          <Button 
            size="lg" 
            variant="outline" 
            className="border-neon-cyan text-neon-cyan hover:bg-neon-cyan/10 transition-all duration-300 hover:scale-105 group"
          >
        ...
        <div className="flex justify-center space-x-8 animate-fade-in">
          {[
            { icon: Github, href: '#', label: 'GitHub', color: 'hover:text-neon-purple' },
            { icon: Linkedin, href: '#', label: 'LinkedIn', color: 'hover:text-neon-cyan' },
            { icon: Mail, href: '#', label: 'Email', color: 'hover:text-neon-green' },
          ].map((social, index) => (
            <a
              key={social.label}
              href={social.href}
              className={`p-4 glass-card ${social.color} transition-all duration-300 hover:scale-110 group relative overflow-hidden`}
        ...
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce">
          <div className="w-6 h-10 border-2 border-neon-purple/50 rounded-full flex justify-center relative overflow-hidden">
            <div className="w-1 h-3 bg-neon-purple rounded-full mt-2 animate-pulse"></div>
            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-neon-cyan/30 to-transparent animate-wave"></div>
          </div>
          <p className="text-xs text-muted-foreground mt-2 font-mono">Scroll to explore</p>
        </div>
      </div>
    </section>
  );
